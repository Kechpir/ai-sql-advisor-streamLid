...
with tab_saved:
    ...
    # если пользователь не загрузил актуальную схему в этой сессии — подсказка
    if "schema_json" not in st.session_state:
    st.info("ℹ️ Чтобы использовать «⚙️ Diff» и «♻️ Обновить», сначала во вкладке «🔎 Сканировать/Генерировать» загрузите актуальную схему БД.")
